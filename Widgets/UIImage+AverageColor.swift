...
//  Modified a little bit by John Davis to exclude black color results for aesthetic reasons. 
//
#if os(OSX)
    import AppKit
...
extension UIImageColors {
    public var backgroundOrOther: UIColor? {
        return background ?? primary ?? secondary ?? detail
    }
    
    public var primaryOrOther: UIColor? {
        return primary ?? secondary ?? detail ?? background
    }
    
    public var secondaryOrOther: UIColor? {
        return secondary ?? detail ?? background ?? primary
    }
    
    public var detailOrOther: UIColor? {
        return detail ?? background ?? primary ?? secondary
    }
}
...
extension UIImage {
    ...
    public func getNonBlackColors(quality: UIImageColorsQuality = .high) -> UIImageColors? {
        var colors = getColors(quality: quality)
        
        colors?.background.flatMap { colors?.background = isTooDark($0) ? nil : $0 }
        colors?.primary.flatMap { colors?.primary = isTooDark($0) ? nil : $0 }
        colors?.secondary.flatMap { colors?.secondary = isTooDark($0) ? nil : $0 }
        colors?.detail.flatMap { colors?.detail = isTooDark($0) ? nil : $0 }
        
        return colors
    }
    
    func isTooDark(_ color: UIColor) -> Bool {
        var red: CGFloat = 0
        var green: CGFloat = 0
        var blue: CGFloat = 0
        var alpha: CGFloat = 0
        ...
        return red < (10.0/255.0) && green < (10.0/255.0) && blue < (10.0/255.0)
    }
